(() => {
  ...
  function buildStockItem(isFilterOn, stock, index, stocks) {
    ...
    return `
    <li data-id="${ stock.Symbol }" class="stock-item flex-sb-center">
      <span>${ stock.Symbol } (${ stock.Name })</span>
      <div class="stock-right  flex-sb-center">
        <span>${ Number(stock.LastTradePriceOnly).toFixed(2) }</span>
        <button class='btn-stock  ${getButtonColor(stock)}' data-type="stock-change-btn">${ stockChange }</button>
        <div class="up-down-arrows ${isFilterOn ? 'hidden' : ''}">
          <button data-type="arrow-up-btn" class="icon-arrow arrow-up" ${disabledUp}></button>
          <button data-type="arrow-down-btn" class="icon-arrow arrow-down" ${disabledDown}></button>
        </div>
      </div>
    </li>`;
  }
  ...
  rootElm.addEventListener('click', (e) => {
    ...
    const eventData = {
      dataId: closestAncestor.getAttribute('data-id'),
      dataType: event.target.getAttribute('data-type')
    };
    window.STKR.PubSub.publish('view-clicked', eventData );
  });
